...
public class Perft {
    ...
    public static void perftRoot(long WP, long WN, long WB, long WR, long WQ, long WK, long BP, long BN, long BB, long BR, long BQ, long BK, long EP, boolean CWK, boolean CWQ, boolean CBK, boolean CBQ, boolean WhiteToMove, int depth) {
        String moves;
        if (WhiteToMove) {
            moves = Moves.WhitePossibleMoves(WP, WN, WB, WR, WQ, WK, BP, BN, BB, BR, BQ, BK, EP, CWK, CWQ, CBK, CBQ);
        } else {
            moves = Moves.BlackPossibleMoves(WP, WN, WB, WR, WQ, WK, BP, BN, BB, BR, BQ, BK, EP, CWK, CWQ, CBK, CBQ);
        }
        for (int i = 0; i < moves.length(); i += 4) {
            ...
            if (Character.isDigit(moves.charAt(i + 3))) {//'regular' move
                ...
                if (((1L << start) & WK) != 0) {
                    ...
                    CBKt = false;
                    CBQt = false;
                } else if (((1L << start) & WR & (1L << Moves.CASTLE_ROOKS[0])) != 0) {
                    CWKt = false;
                } else if (((1L << start) & WR & (1L << Moves.CASTLE_ROOKS[1])) != 0) {
                    CWQt = false;
                } else if (((1L << start) & BR & (1L << Moves.CASTLE_ROOKS[2])) != 0) {
                    CBKt = false;
